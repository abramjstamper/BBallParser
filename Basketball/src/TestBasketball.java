...
public class TestBasketball extends BasketballBaseListener {
    ...
    public void exitTurnover(BasketballParser.TurnoverContext ctx) {
        ...
        if (ctx.player(1) != null) {
            int stealPlayer = Integer.parseInt(ctx.player(1).getText().substring(1));
            if (ctx.player(1).getText().substring(0, 1).equals("h")) {
                homeTeamSteal.putIfAbsent(stealPlayer, 0);
                int nextTurnoverNum = homeTeamSteal.get(stealPlayer) + 1;
                homeTeamSteal.put(stealPlayer, nextTurnoverNum);
            } else {
                guestTeamSteal.putIfAbsent(stealPlayer, 0);
                int nextAwayTurnoverNum = guestTeamSteal.get(stealPlayer) + 1;
                guestTeamSteal.put(stealPlayer, nextAwayTurnoverNum);
            }
        }
    }
    ...
    public void exitFreethrow(BasketballParser.FreethrowContext ctx) {
        ...
        int playerNum = Integer.parseInt(tempParse.substring(1));
        if (tempParse.substring(0, 1).equals("h")) {
            if (ctx.getText().substring(0, 1).equals("m")) {
    ...
    public void prettyPrint() {
        ...
        System.out.println("Guest: " + (gfgm + gftm));
        System.out.println("Home: " + (hfgm + hftm));
    }
